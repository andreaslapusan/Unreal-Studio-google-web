 ...
 *   ANTES: 14d (b14), 7d (b7), 2d (b2), 1d (b1) y el día del vencimiento (due).
 *   DESPUÉS: diario los días 1–7 (a1…a7) y luego cada 7 días (a14, a21…)
...
function stageFor(daysUntil, lang) {
  ...
  if (daysUntil === 14) return { key: "b14", subject: tr(lang,"b14_subject"), lead: tr(lang,"b14_lead"), when: tr(lang,"b14_when"), tone: "calm" };
  ...
  if (daysUntil === 1) return { key: "b1", subject: tr(lang,"b1_subject"), lead: tr(lang,"b1_lead"), when: tr(lang,"b1_when"), tone: "calm" };
  if (daysUntil === 0) return { key: "due", subject: tr(lang,"due_subject"), lead: tr(lang,"due_lead"), when: tr(lang,"due_when"), tone: "alert" };
  // DESPUÉS del vencimiento
  if (daysUntil === -1) return { key: "a1", subject: tr(lang,"a1_subject"), lead: tr(lang,"a1_lead"), when: tr(lang,"a1_when"), tone: "alert" };
  if (daysUntil === -3) return { key: "a3", subject: tr(lang,"a3_subject"), lead: tr(lang,"a3_lead"), when: tr(lang,"a3_when"), tone: "alert" };
  // Días 2 y 4–7 tras el vencimiento: aviso diario genérico.
  if (daysUntil <= -2 && daysUntil >= -7) { const n = Math.abs(daysUntil); return { key: `a${n}`, subject: tr(lang,"aN_subject",{n}), lead: tr(lang,"aN_lead",{n}), when: tr(lang,"aN_when",{n}), tone: "alert" }; }
  // A partir de ahí, cada 7 días (14, 21, 28…) hasta que se marque pagado.
  if (daysUntil <= -14 && Math.abs(daysUntil) % 7 === 0) { const n = Math.abs(daysUntil); return { key: `a${n}`, subject: tr(lang,"aN_subject",{n}), lead: tr(lang,"aN_lead",{n}), when: tr(lang,"aN_when",{n}), tone: "alert" }; }
  return null;
}
