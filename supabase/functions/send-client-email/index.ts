...
const FOOTER_TR: Record<string, { sendOnly: string; changeLang: string }> = {
  es: { sendOnly: "Correo de solo envío · no respondas a este mensaje.", changeLang: "Para cambiar el idioma, entra en tu portal de cliente." },
  en: { sendOnly: "Send-only address · please don't reply to this message.", changeLang: "To change the language, log in to your client portal." },
  ro: { sendOnly: "Adresă doar pentru trimitere · te rugăm să nu răspunzi la acest mesaj.", changeLang: "Pentru a schimba limba, accesează portalul tău de client." },
  id: { sendOnly: "Email khusus pengiriman · mohon jangan membalas pesan ini.", changeLang: "Untuk mengubah bahasa, masuk ke portal klien Anda." },
};
function brandWrap(subject: string, inner: string, lang = "es"): string {
  const year = "2026";
  const L = (lang || "es").slice(0, 2);
  const F = FOOTER_TR[L] || FOOTER_TR.es;
...
</body></html>`.replace(/>\s+</g, "><").replace(/\s{2,}/g, " ").replace(/<p style="(?![^"]*text-align)/g,'<p style="text-align:justify;').trim();
}
...
Deno.serve(async (req) => {
  ...
  let p: { adminUserId?: string; to?: string; subject?: string; html?: string; kwitansiId?: string; lang?: string };
  ...
  try {
    await client.send({
      ...
      html: brandWrap(p.subject, p.html, p.lang),
      headers: UNSUB,
    });
  } catch (e) {
