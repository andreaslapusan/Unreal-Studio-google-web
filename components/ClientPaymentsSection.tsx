 ...
 *
 * Modos:
 *  - Sin filtro: muestra todas las unidades (uso antiguo).
 *  - compact + filterName/filterUnit: muestra SOLO el calendario de UNA propiedad,
 *    para incrustarlo debajo de su tarjeta en "Mis Inversiones".
...
const ClientPaymentsSection: React.FC<Props> = ({ clientId, filterName, filterUnit, compact }) => {
  ...
  const body = (
    <div className={compact ? 'space-y-4' : 'space-y-6'}>
      {shown.map((u) => {
        ...
        return (
          <article key={u.client_project_id} className={compact ? '' : 'bg-white rounded-2xl border border-primary/5 shadow-sm p-6'}>
            <div className="flex items-center justify-between mb-3">
              {compact
                ? <h4 className="text-[10px] font-black uppercase tracking-widest text-primary/40">Calendario de pagos</h4>
                  ...
                  <li key={p.id} className={`rounded-lg p-3 text-sm ${p.received ? 'bg-green-50 border border-green-200' : overdue ? 'bg-red-50 border border-red-200' : 'bg-almond/40 border border-primary/10'}`}>
                    ...
                    {!p.received && (
                      <div className="mt-2 pl-9">
                        {claimed ? (
                          <span className="text-xs font-bold text-green-700 inline-flex items-center gap-1"><span className="material-symbols-outlined text-sm">check</span> Aviso enviado. Lo verificaremos y te confirmaremos.</span>
                        ) : claimingId === p.id ? (
  ...
  if (compact) {
    return <div className="mt-6 pt-6 border-t border-gray-100">{body}</div>;
  }
  return (
